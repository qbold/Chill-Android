...
public final class PngUtils {
    ...
    public static void toIndexed256Colors(InputStream input, OutputStream output) throws IOException {
        ...
        if (channel == 1) {
            for (int row = 0, rows = pnginfo1.rows; row < rows; row++) {
                ...
                for (int col = 0, len = scanline.length / channel, offset = 0; col < len; col++, offset = col * channel) {
                    indexedLine[row][col] = builder.getIndex(scanline[offset], 0, 0, 255);
                }
            }
        } else if (channel == 2) {
            for (int row = 0, rows = pnginfo1.rows; row < rows; row++) {
                ...
                for (int col = 0, len = scanline.length / channel, offset = 0; col < len; col++, offset = col * channel) {
                    indexedLine[row][col] = builder.getIndex(scanline[offset], scanline[offset + 1], 0, 255);
                }
            }
        } else if (channel == 3) {
            for (int row = 0, rows = pnginfo1.rows; row < rows; row++) {
                ...
                int[] scanline = ((ImageLineInt) l1).getScanline();
                for (int col = 0, len = scanline.length / channel, offset = 0; col < len; col++, offset = col * channel) {
                    indexedLine[row][col] = builder.getIndex(scanline[offset], scanline[offset + 1], scanline[offset + 2], 0);
                }
            }
        } else if (channel == 4) {
            for (int row = 0, rows = pnginfo1.rows; row < rows; row++) {
...
//                        throw new IllegalArgumentException("sorry png image which has alpha value is not supported");
                    ...
                    indexedLine[row][col] = builder.getIndex(scanline[offset], scanline[offset + 1], scanline[offset + 2], scanline[offset + 3]);
                }
            }
        }
        ...
        ImageInfo pnginfo2 = new ImageInfo(png1.imgInfo.cols, png1.imgInfo.rows, bitDepth, true, false, true);
